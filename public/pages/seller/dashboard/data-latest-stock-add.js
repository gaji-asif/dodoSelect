...
$(document).ready(function () {
    fetchLatestStockAddProducts();
});

const fetchLatestStockAddProducts = () => {
    $.ajax({
        type: 'GET',
        url: route('dashboard.data.latest-stock-products', { type: 'add' }),
        success: function (response) {
            const data = response.data;
            const latestStockProducts = data.latest_stock_products;

            $('#__latestStockProductAddWrapper').html(null);
            $('#__latestStockProductAddWrapper').append(
                $('<div/>', {
