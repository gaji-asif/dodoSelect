...
$('body').on('click', '.btn__sync-selected', function () {
    const selectedRows = $('#wooProductTable').DataTable().column(0).checkboxes.selected();
    ...
    $.ajax({
        ...
        success: function (response) {
            $('#__alertSuccessWooProductTable').removeClass('hidden').find('.alert-content').html(response.message);
        ...
        error: function (error) {
            ...
            $('#__alertDangerWooProductTable').removeClass('hidden').find('.alert-content').html(response.message);
