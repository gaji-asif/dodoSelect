...
use App\Http\Requests\Shopee\LinkedCatalog\StoreRequest;
...
class LinkedCatalogController extends Controller
{
     ...
     *
     * @param  \App\Http\Requests\Shopee\LinkedCatalog\StoreRequest  $request
     * @return \Illuminate\Http\Response
     */
    public function store(StoreRequest $request)
    {
        try {
            $shopee_product_id = $request->shopee_product_id;
            $product_id = $request->product_id;

            $shopeeProduct = ShopeeProduct::where('id', $shopee_product_id)->first();
            $shopeeProduct->dodo_product_id = $product_id;
            $shopeeProduct->is_linked = ShopeeProduct::IS_LINKED_YES;
            $shopeeProduct->save();
                ...
                ->addColumn('product_details', function ($product) {
                    return '
                        <div class="grid grid-cols-1 gap-2">
                            ...
                            <div>
                                <button type="button"
                                    class="btn-action--yellow btn-link-shopee-product"
                                    data-product-id="'. $product->id .'"
                                    onClick="linkCatalogToShopeeProduct(this)">
