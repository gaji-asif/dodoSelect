...
class ShopeeOrderAirwayBillDownload3 implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if ($this->process_type="initiate") {
            ...
            /* Sort out the orders according to the shops they belong to. */
            ...
            foreach ($shopee_shops as $id => $shop_id) {
                ...
                if(sizeof($shop_specific_ordersn_list[$id]) > 0) {
                    $loop_counter = floor(sizeof($shop_specific_ordersn_list[$id])/$this->shopee_limit);
                    if (sizeof($shop_specific_ordersn_list[$id])%$this->shopee_limit > 0) {
                        $loop_counter += 1;
                    }
                    for ($i=0; $i<$loop_counter; $i++) {
                        $ordersn_arr = array_slice($shop_specific_ordersn_list[$id], $i*$this->shopee_limit, $this->shopee_limit);
                        $response = $this->getAirwayBillInfoFromShopee($shop_id, $ordersn_arr);
                        if (isset($response, $response["request_id"], $response["batch_result"], $response["batch_result"]["airway_bills"])) {
                            ...
                            if (sizeof($airway_bills) > 0) {
                                ...
                                $this->updateShopeeOrderPurchaseDownloadInfo($ordersn_arr);
                                if ($i==0) {
