...
class ShopeeOrderInitWithAddressId implements ShouldQueue
{
     ...
     * @return void
     */
    public function __construct($shopee_shop_id, $address_id, $time_slot_id, $selected_time_slot, $ordersn, $auth_id)
    {
    ...
    public function handle()
    {
            ...
            if (isset($orderPurchase)) {
                ...
                if (isset($response["request_id"])) {
                    ...
                    if(!isset($response["error_param"]) and !isset($response["error_params"]) and !isset($response["error_not_exist"]) and
                        ...
                        if (isset($this->selected_time_slot) and !empty($this->selected_time_slot)) {
                            $orderPurchase->pickup_shipped_on = $this->selected_time_slot;
                        }
                        ...
                        $orderPurchase->shopee_shipping_method = strtolower(ShopeeOrderPurchase::SHIPPING_METHOD_PICKUP);
        ...
        if ($this->matchLastLastShopeeOrderIdForBulkInitLock($this->ordersn, $this->auth_id)) {
            Log::debug("Removing lock for batch init for multiple shops (".$this->ordersn.")");
