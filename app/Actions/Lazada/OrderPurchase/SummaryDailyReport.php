...
class SummaryDailyReport
{
    ...
    public function handle(string $dateParameter = null)
    {
        ...
        $currentDayOrdersByShop = LazadaOrderPurchase::query()
            ->selectRaw("website_id, SUM(price) AS total_amount, COUNT(id) AS total_orders")
        ...
        $monthToDateOrders = LazadaOrderPurchase::query()
            ->selectRaw("SUM(price) AS total_amount, COUNT(id) AS total_orders")
            ->whereBetween('order_date', [ $monthToDateFirstDate . ' 00:00:00', $date . ' 23:59:59' ])
            ->first();

        $lastMonthOrders = LazadaOrderPurchase::query()
            ->selectRaw("SUM(price) AS total_amount, COUNT(id) AS total_orders")
