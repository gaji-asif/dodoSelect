...
<div class="col-6">
 <div class="card shipping_address_wraps mb-3">
    ...
    <div class="form-group">
      ...
      <br/>
      <span class="mt-2" id="shipping_phone" style="font-weight: bold;"  class="thaiFont"> {{ $shipping['shipping_phone'] }} </span>
      @endif
...
<div class="order_details_div" id="order_details_div">
 <h6 class="pt-4"><strong class="order_shipment_color mb-2">Shipped Product Details:</strong></h6>

 @if (isset($shipmentWithProducts))
 <div class="full-card show">
 	<table class="table table-responsive">
 		...
 		<tbody class="table-body">
 			@if (isset($shipmentWithProducts))
        @foreach ($shipmentWithProducts as $row)
        <tr class="new" id='@if (isset($row->product_id)){{($row->product_id)}} @endif ' >
          ...
          @php 
            ...
            @if(isset($images[0]->src))
                <img src="{{ $images[0]->src}}" height="80" width="80" alt="" style="margin-top: 13px;" >
            @else
...
<script type="text/javascript">
 $(document).ready(function(){
  $(document).on('click', '#customers_details_btn', function() {
    $(".customers_details_div").show();
    $(".order_details_div").hide();
  });

  $(document).on('click', '#order_details_btn', function() { 
