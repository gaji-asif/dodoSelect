<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/sweetalert2@11.0.19/dist/sweetalert2.min.css">
  ...
  <form action="{{ route('shipment.updateForWCCustomShipment') }}" method="post" id="__formUpdateShipment">
    <input type="hidden" name="order_id" id="__order_idCreateShipment" value="{{$editData->order_id}}">
      ...
      <div class="grid grid-cols-1 gap-4">
        <div class="row">
          <div class="col-lg-4">
            ...
            <x-input type="text" value="{{$editData->order_id}}" id="__order_id_displayCreateShipment" class="bg-gray-200" readonly />
          </div>
  ...
  <div id="ordered_products">
     ...
     <tbody class="table-body new_products_table" id="new_products_table_for_edit">
      ...
      <tr class="new" id="product_{{($row->product_id)}}">
        ...
        <td>
          ...
          <div class="row">
            <div class="col-lg-4 col-sm-12">
              ...
              <input type="text" id="shipment_qty_{{$row->id}}" name="shipment_qty[]" data-id="{{$row->id}}" class="form-control rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 mt-1 shipment_qty" value="{{$row->quantity}}">
             </div>
             <div class="col-lg-4 col-sm-12">
              <button type="button" class="btn btn-danger font_size_12 margin_top_re" onclick="removeProductWrapperForEdit('{{$row->product_id}}');">
                REMOVE
              </button>
             </div>
          </div>
