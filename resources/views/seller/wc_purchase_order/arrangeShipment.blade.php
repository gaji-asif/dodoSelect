...
<form action="{{ route('WCshipment.arrangeForOrder') }}" method="post" id="__formArrangeShipment">
    <input type="hidden" name="order_id" id="__order_idCreateShipment" value="{{$orderManagement->order_id}}">
    <input type="hidden" name="website_id" id="__order_website_id" value="{{$orderManagement->website_id}}">
    <div class="mb-3">
        <div class="grid grid-cols-1 gap-4">
            <div class="row">
                <div class="col-lg-4 mb-3">
                    ...
                    <x-input type="text" value="{{$orderManagement->order_id}}" id="__order_id_displayArrangeShipment" class="bg-gray-200" readonly />
                </div>
                <div class="col-lg-4">
                                ...
                                <tr class="new" id='@if (isset($row->product)){{($row->sku)}} @endif ' >
                                    ...
                                    if(!empty($arrProductImageWithID[$row->product_id])){
                                        ...
                                        $images = json_decode($product->images);
                                         ...
                                         $product = \App\Models\WooProduct::where(['product_code' => $row->sku])->first();
                                            if (!empty($product)) {
                                                $images = json_decode($product->images);
                                    ...
                                    @if(isset($image_url) && $image_url != '')
                                        <img src="{{ $image_url}}" height="80" width="80" alt="" style="margin-top: 13px;" >
                                    @else
                                                ...
                                                <div class="lg:col-span-3">
                                                    <div class="text-left">
                                                        <div class="mb-1">
                                                            ...
                                                            <input type="hidden" id="shipment_qty_{{$row->id}}" name="shipment_qty[]" data-id="{{$row->id}}" value="{{$row->quantity-$shipped_qty}}" placeholder="0" class="form-control rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 mt-1 shipment_qty">
    ...
    <div class="pb-5 text-center">
        ...
        <x-button type="submit" color="blue" id="__btnSubmitArrangeShipmentForOrder">
            {{ __('Arrange Shipment') }}
        </x-button>
    </div>
</form>
...
<script type="text/javascript">
    ...
    $('#__btnSubmitArrangeShipmentForOrder').on('click', function(event) {
        event.preventDefault();
        $(this).prop('disabled', true);
        var orderID = $("#__order_id_displayArrangeShipment").val();

        const shipmentStoreUrl = $("#__formArrangeShipment").attr('action');
        const formData = new FormData($("#__formArrangeShipment")[0]);
        $.ajax({
            type: 'POST',
            ...
            success: function(responseData) {
                ...
                if(responseData){
                    var order_id = $("#__order_id_displayArrangeShipment").val();
                    var website_id = $("#__order_website_id").val();
    ...
    $('#__shipment_dateCreateShipment').on("click", function(){
        $("#__btnSubmitArrangeShipmentForOrder").prop('disabled', false);
    });
