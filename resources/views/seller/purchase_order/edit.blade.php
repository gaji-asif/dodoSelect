<x-app-layout>
    ...
    @if(\App\Models\Role::checkRolePermissions('Can access menu:Purchase Order - Purchase Order'))
        ...
        <x-card.card-default>
            <x-card.header>
                ...
                <x-card.title>
                    {{ __('translation.Edit Purchase Order') }}
                </x-card.title>
            ...
            <x-card.body>
                ...
                <form method="POST" action="{{ route('order_purchase.update', [ 'order_purchase' => $orderPurchase ]) }}" id="__formPurchaseOrder" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="_method" value="PUT">
					 <input type="hidden" id="order_purchase_id" name="order_purchase_id" value="{{ $orderPurchase->id }}">
                        ...
                        <div class="tab-pane active" id="po-info">
                            <div class="mb-10">
								...
								<div class="w-full mb-5">
									<x-label>
										{{ __('translation.Supply From') }} <x-form.required-mark/>
									</x-label>
									<div class="flex flex-row">
										<div>

											<input type="radio" name="check" id="__importSupplyFrom" value="1" @if($orderPurchase->supply_from=='1') checked="true" @endif >
												{{ __('translation.Import') }}

										</div>
										<div class="ml-5">
											<input type="radio" name="check" id="__domesticSupplyFrom"  value="2"  @if($orderPurchase->supply_from=='2') checked="true" @endif>
								...
								<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-x-8">
									<div class="sm:col-span-2 lg:col-span-4 grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-x-8">
										<div class="lg:col-span-2">
											...
											<x-input type="text" name="order_date" id="order_date" class="datepicker-1" value="{{ !empty($orderPurchase->order_date) ? date('d-m-Y', strtotime($orderPurchase->order_date)) : '' }}" />
										</div>
										...
										<div class="lg:col-span-2">
											...
											<x-select name="supplier_id"  id="supplier_id" style="width: 100%">
												...
												@foreach ($suppliers as $supplier)
													<option value="{{ $supplier->id }}" @if($orderPurchase->supplier_id == $supplier->id) selected @endif>
									...
									<div class='imports'>
										<x-label>
												{{ __('translation.Ship Type') }} <x-form.required-mark/>
											</x-label>
											<x-select name="shipping_type_id" id="shipping_type_id" style="width: 100%">
												...
												@foreach ($shiptypes as $shiptype)
													<option value="{{ $shiptype->id }}" @if($orderPurchase->shipping_type_id == $shiptype->id) selected @endif>
														{{ $shiptype->name }}
													</option>
												@endforeach
											</x-select>
									...
									<div class='imports'>
										<x-label>
												{{ __('translation.Cargo') }} <x-form.required-mark/>
											</x-label>
											<x-select name="agent_cargo_id" id="agent_cargo_id" style="width: 100%">
												...
												@foreach ($cargos as $cargo)
													<option value="{{ $cargo->id }}" @if($orderPurchase->agent_cargo_id == $cargo->id) selected @endif>
														{{ $cargo->name }}
													</option>
												@endforeach
											</x-select>
                                    ...
                                    <div class='imports'>
										<x-label>
											{{ __('translation.Shipping Mark') }} <x-form.required-mark/>
										</x-label>
										<x-select name="shipping_mark_id" id="shipping_mark_id" style="width: 100%">
											<option disabled selected value="0">
                                            - {{ __('translation.Select Shipping Mark') }} -
                                        </option>
                                        @foreach ($shipping_marks as $shipping_mark)
                                            <option value="{{ $shipping_mark->id }}" @if($orderPurchase->shipping_mark_id == $shipping_mark->id) selected @endif>
                        ...
                        <div class="tab-pane" id="shipment-details">
                            <div class="mb-10">
                                ...
                                <div class="mb-10" id="shipment_wrapper_list">
                                    <div class="text-center text-xl" id="processing"></div>
                                    <div class="overflow-x-auto">
						...
						<div class="mb-10">
							<div class="flex flex-row items-center justify-between mb-3 mt-3">
								...
								@php
									...
									$exchange_rate_id = $payment->exchange_rate_id ?? 0;
									$payment_status = $payment->payment_status ?? '';
									$payment_bank_account = $payment->bank_account ?? '';
									$payment_notes = $payment->notes ?? '';
									$payment_file_invoice = $payment->file_invoice ?? '';
									$payment_file_payment = $payment->file_payment ?? '';
								@endphp

								<x-input type="hidden" name="payment_id" id="payment_id" value="{{ $payment_id }}" style="width: 100%"></x-input>

									<div>
										<x-label>
										{{ __('translation.Amount Invoice') }}
										</x-label>
										<x-input type="number" name="amount" id="amount" value=""  style="width: 100%">
										</x-input>
									</div>
									...
									<div>
										...
										<x-input type="number" name="paid" id="paid" value="{{ $payment_paid }}" style="width: 100%">
										</x-input>
									</div>
									...
									<div>
										...
										<x-select name="payment_exchange_rate_id" id="payment_exchange_rate_id">
											<option value="" selected>
												{{ '- ' . __('translation.Select Currency') . ' -' }}
											</option>
											@foreach ($exchangeRates as $exchangeRate)
												<option value="{{ $exchangeRate->id }}" @if($exchange_rate_id ==  $exchangeRate->id) selected @endif>
													{{ $exchangeRate->name }}
												</option>
											@endforeach
										</x-select>
								...
								<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4 sm:gap-x-8 mt-4">
									 ...
									 <div>
										...
										<x-textarea name="bank_account" id="bank_account" class="bank_account">{{ $payment_bank_account }}</x-textarea>
									</div>
									...
									<div>
										...
										<x-textarea name="notes" id="notes" class="notes">{{ $payment_notes }}</x-textarea>
									</div>
									...
									<div>
										...
										<x-input type="file" name="file_invoice" id="file_invoice" style="width: 100%">
										</x-input>
										@if($payment_file_invoice)
										<img src="{{asset($payment_file_invoice)}}" style="max-width:100px;">
										@endif
									</div>
									...
									<div>
										...
										@if($payment_file_payment)
										<img src="{{asset($payment_file_payment)}}" style="max-width:100px;">
										@endif
									</div>
								</div>
						</div>
						...
						<div>
							...
							<div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
								<div>
									...
									<div class="w-full">
										<x-input type="number" name="product_price[]" value="{product_price}" min="0" step="0.001" />
						...
						<div class="md:col-span-1 lg:col-span-1">
							<div class="mb-4 lg:mb-2 " style="border: 1px solid #e6e6e6;padding:20px;">
								...
								<div class="w-full md:w-full md:pr-2">
									<x-input type="number" name="product_quantity[]"  data-code="{product_code}" class="order-qty__field order-qty__field_{product_code} mb-2" value="{total_incoming}" min="0" />
					...
					<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-2 gap-4 sm:gap-x-6 lg:pt-1">
						...
						<div class="grid grid-cols-1 lg:grid-cols-1 gap-3">
							...
							<div class="mb-4 lg:mb-2 " style="border: 1px solid #e6e6e6;padding:20px;">
								...
								<div class="w-full md:w-full md:pr-2">
									<x-input data-code="{product_code_shipped}" data-product_id="{product_id_shipped}" id="ship_quantity_{product_code_shipped}" class="mb-2 ship_quantity" type="number" name="ship_quantity[]" data-old-qty="{shipQtyOld}" max="{available_shipped_qty}" pack="{pack}"  value="{shipQty}"  />
    ...
    @push('bottom_js')
        ...
        <script>
            ...
            $(window).on('load', function() {
                ...
                const orderPurchase = {!! $orderPurchase->toJson() !!};
                //console.log(orderPurchase);
                const order_date = orderPurchase.order_date;
        ...
        <script>
			...
			// This will fetch and store product details
            ...
            const displayProductsIntoToCreateForm = (all_products) => {
                ...
                $.each(all_products, function (key, product) {
                    ...
                    let templateProductItemElement = $('#__templateProductItem').clone();
                        ...
                        templateProductItemElement.html(function(index, html) {
                            return html.replaceAll('{product_price}', product.product_price ? product.product_price : 0);
                        });
            ...
            $(document).on('click', '.__confirmNewShipment', function(event) {
                ...
                for(var i = 0; i < ship_quantites.length; i++){
                    ...
                    supplier = {
							...
							supplier_name : $("#shipping_type_id option:selected" ).text()
                    }
    ...
    const loadShipmentTable = (supplierId,order_purchase_id) => {
        $("#processing").html("Data is Processing.......");
        $.ajax({
                type: 'GET',
                data: {
                    supplierId: supplierId,
                ...
                url: '{{ route('shipment_data_table_on_po_edit_page') }}',
                success: function(responseJson) {
                    $('#__wrapper_NoProduct').hide();
                    let templateProductItemElement = $('#__templateProductItem').clone();
                    let poshipmentsWithDetails = responseJson;
                    //console.log(poshipmentsWithDetails);
                    $.each(poshipmentsWithDetails, function (key, item) {
                        //allShippings.push(item);
                    });
                    $("#processing").html(" ");

                    loadShipmentDataTable(poshipmentsWithDetails);
            ...
            if(undefined !== item.supplier.supplier_name){
                ...
                var order_date = '<span class="whitespace-nowrap cursor-pointer">';
                        order_date += '<a href="#"class="underline text-blue-500 font-bold">Order No: # '+order_no+'</a><br>';
            ...
            loadShipmentTable(supplierId,order_purchase_id);
			...
			$('.__closeModalShipment').click(function() {
               $('.modal-shipment').addClass('modal-hide');
			   $('body').removeClass('modal-open');
			   $('#add_shipment_wrapper_new').addClass("hide");
            });
        </script>
    @endpush
