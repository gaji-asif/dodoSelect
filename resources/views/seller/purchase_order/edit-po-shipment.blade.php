...
<div class="mt-4" style="width: 100%; float: left;  margin-bottom: 10px; margin-right: 1%;">
    ...
    <input type="hidden" name="order_purchase_id" id="order_purchase_id" value="{{ $order_purchase_id }}">
</div>
        ...
        <div>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-x-8">
                <div class="imports">
                    <x-label>
                        ...
                        <x-input type="text" name="factory_tracking" id="factory_tracking" value="{{ $po_shipment['factory_tracking'] }}" />
                </div>
                ...
                <div>
                    ...
                    <x-input type="text" name="number_of_cartons" id="number_of_cartons" value="{{ $po_shipment['number_of_cartons'] }}" />
                </div>
                ...
                <div>
                    ...
                    <x-input type="text" name="ship_date" id="ship_date1" class="datepicker-1" value="{{ !empty($po_shipment['ship_date']) ? date('d-m-Y', strtotime($po_shipment['ship_date'])) : '' }}" />
                </div>
                ...
                <div>
                    ...
                    <x-input type="text" name="e_a_d_f" id="e_a_d_f1" class="datepicker-1" value="{{ !empty($po_shipment['e_a_d_f']) ? date('d-m-Y', strtotime($po_shipment['e_a_d_f'])) : '' }}" />
                </div>
                <div>
                    <x-label>
                        {{ __('translation.Estimated Arrival Date') }} <span class="font-bold">{{ __('translation.To') }}</span>
                    </x-label>
                    <x-input type="text" name="e_a_d_t" id="e_a_d_t1" class="datepicker-1" value="{{ !empty($po_shipment['e_a_d_t']) ? date('d-m-Y', strtotime($po_shipment['e_a_d_t'])) : '' }}" />
                </div>
				...
				<div>
					...
					<select class="block w-full h-10 px-2 py-2 rounded-md shadow-sm border border-solid border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 mt-1 bg-white form-control" id="exampleFormControlSelect1" name="status">
            ...
            <div class="imports">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-x-8">
                    ...
                    <div>
                        ...
                        <x-input type="text" name="cargo_ref" id="cargo_ref" value="{{ $po_shipment['cargo_ref'] }}" />
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <x-select name="domestic_shipper_id" id="domestic_shipper_id" style="width: 100%">
                ...
                <div>
                    ...
                    <x-input type="text" name="domestic_logistics" id="domestic_logistics" value="{{ $po_shipment['domestic_logistics'] }}" />
                </div>
		...
		@foreach ($po_shipment['po_shipment_details'] as $key=>$pos_detail)
			<x-input type="hidden" name="po_shipment_id" class="mb-2" value="{{$pos_detail['po_shipment_id']}}" />
            ...
            <x-input type="hidden" name="product_id[]" class="mb-2" value="{{$pos_detail['product']['id']}}" />
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-2 gap-4 sm:gap-x-6 lg:pt-1">
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-3 gap-3">
                                <div>
                                    ...
                                    <span class="font-bold lg:block" id="order_qty_{{$pos_detail['product']['product_code']}}">
                                        {{ isset($po_shipment['order_purchase_details'][$key]['quantity']) ? $po_shipment['order_purchase_details'][$key]['quantity'] : ' '}}
                                    </span>
                                </div>
                                <div>
                                    <label class="mb-0">
                                ...
                                <div>
                                    ...
                                    {{ isset($po_shipment['order_purchase_details'][$key]['quantity']) ? ($po_shipment['order_purchase_details'][$key]['quantity']-$total_shipped_qty) : ' '}}
                                    </span>
                                </div>
                            </div>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-1 gap-3">
                            ...
                            <div class="mb-4 lg:mb-2 " style="border: 1px solid #e6e6e6;padding:20px;">
								...
								<span hidden id="pieces_per_pack_{{$pos_detail['product']['product_code']}}">
                                @php 
                                if(!empty($pos_detail['product']['product_cost_details']['pieces_per_pack'])) {
                                   $pieces_per_page = $pos_detail['product']['product_cost_details']['pieces_per_pack'];
                                }else{
                                    $pieces_per_page = 0;
                                }
                                @endphp
                                    ...
                                    <x-input type="number" name="ship_quantity[]" class="ship_quantity mb-2" data-old-qty="{{$pos_detail['ship_quantity']}}" value="{{$pos_detail['ship_quantity']}}" min="1" data-code="{{$pos_detail['product']['product_code']}}" />
                                    <strong>{{ __('translation.Total Pieces') }} : <span class='res_pieces'>{{$pieces_per_page * $pos_detail['ship_quantity']}}</span></strong>
                                </div>
                            </div>
