<script type="text/javascript">
 ...
 // open discount modal
 function discount_pricess(product_price){
      $("#discount_price_modal").modal('show');
      $("#discount_price").val(null);
      var price_code = product_price.split("/");
      //$(".pr_"+price_code[1]).text('Dicount Price:'+price_code[0]);
      $("#current_price").val(price_code[0]);
      $("#current_product").val(price_code[1]);
 }
            ...
            if($(this).val() !== "" )
            {
              ...
              .done(function(data) {
                  ...
                  if(data.product_code !== '')
                  {
                    ...
                    inputform = `<input type="number" class="here_quantity order_quantity product_need" name="product_quantity[]" min='1' style="width:50px" class="form-control rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 mt-1" value='1'>`;
                    ...
                    <td width="90px;"><img height="70" width="70" src="${currentUrl}/public/${product_image}" class="cutome_image" alt="">
                    </td>
  ...
  function calculateTotals() {
    ...
    if(shippingCost !== ''){
        ...
        function calculateWeight(row) {
          ...
          total = Number(input)*Number(input_count);
...
$(document).ready(function(){
 ...
 $(function(){
        $("#create_order").submit(function(event){
            event.preventDefault();
            var select_product = document.getElementsByName('product_id[]');
            //alert(select_product.length);
            ...
            if(select_product.length>0){
                if($("#contact_phone_main").val().length === 0){
                    alert("please add your Customer details");
                    return false;
                }

               var shipping_methods_val = $('input[name="shipping_methods"]:checked').val();
               ...
               $.ajax({
                    url: '{{route('storeOrderData')}}',
                    type:'POST',
                    data:$(this).serialize(),
                    success:function(result){
                      // alert(result);
                       $("#ordered_summary").modal('show');
                       $("#selected_customers").html('');
                       $(".order_summ").html(result);
