<x-app-layout>
    ...
    @if(\App\Models\Role::checkRolePermissions('Can access menu: Lazada - Settings'))
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="w-full overflow-x-auto">
                            <table class="table-auto border-collapse w-full border mt-4" id="lazada_shop">
                                ...
                                <tr>
                                    ...
                                    <th class="px-4 py-2 border-2">Access token</th>
                                    <th class="px-4 py-2 border-2">Refresh token</th>
                                ...
                                @if (isset($shops))
                                    @foreach ($shops as $key=>$row)
                                        <tr>
                                            ...
                                            <td>
                                                @if (isset(json_decode($row->response)->expires_in)) 
                                                    Will expire in {{ \Carbon\Carbon::now()->diffInDays(\Carbon\Carbon::now()->addSeconds(json_decode($row->response)->expires_in)) }} days
                                                @endif
                                            </td>
                                            <td>
                                                @if(isset(json_decode($row->response)->refresh_expires_in))
                                                    Will expire in {{ \Carbon\Carbon::now()->diffInDays(\Carbon\Carbon::now()->addSeconds(json_decode($row->response)->refresh_expires_in)) }} days</td>
                                                @endif                                        
                                            <td>
                                                <button type="button" title="Refresh token" class="btn btn-sm bg-warning text-white rounded px-2 py-1 capitalize cursor-pointer" data-toggle="modal" data-target="#refreshModal{{$row->id}}" id="BtnRefresh">
                                                ...
                                                <div class="modal fade" id="editModal{{$row->id}}" tabindex="-1" role="dialog" aria-hidden="true">
                                                    <div class="modal-dialog" role="document">
                                                        <div class="modal-content">
                                                            <form method="POST" action="{{ route('lazada.update.shop',['id'=>$row->id]) }}" id="form-import" enctype="multipart/form-data">
                                                ...
                                                <div class="modal fade" id="refreshModal{{$row->id}}" tabindex="-1" role="dialog" aria-hidden="true">
                                                    <div class="modal-dialog" role="document">
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                            ...
                                                            <div class="modal-body">
                                                                <div>
                                                                    <h6>Do you want to refresh token</h6>
                                                                </div>
                                                            </div>
                                                            <div class="modal-footer">
                                                                ...
                                                                <form method="POST" action="{{ route('lazada.refresh.token', [$row->id]) }}">
                                                                    @csrf
                                                                    <button type="submit" class="btn bg-warning text-white">Yes</button>
                                                                </form>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="modal-dialog" role="document">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-footer">
                                                                ...
                                                                <form method="POST" action="{{ route('lazada.delete.shop', [$row->id]) }}">
                                                                    @csrf
    ...
    <!-- Add shop modal -->
    <div class="modal-insert @if(!request()->get('code')) modal-hide @endif">
        <div style="background-color: rgba(0,0,0,0.5)"
            ...
            <div class="bg-white w-11/12 md:max-w-md mx-auto rounded shadow-lg pt-4 pb-6 text-left px-6"
                ...
                <div class="flex justify-between items-center pb-3">
                    <p class="text-2xl font-bold">Add shop</p>
                ...
                <form method="POST" action="{{ route('lazada.add.shop') }}" id="form-insert">
                    @csrf
                    <div>
                        <x-label> Shop name </x-label>
                        <x-input type="text" name="shop_name" id="shop_name" :value="old('shop_name')" required></x-input>
                    </div>
                    <x-input type="hidden" name="code" id="code" :value="request()->get('code')" required></x-input>
                    <x-input type="hidden" name="shop_id" id="shop_id" :value="0" required></x-input>
        ...
        <script>
            $(document).ready(function() {
                $('#BtnAuth').click(function() {
                    window.location = '{{route('lazada.authorization')}}'
                });
