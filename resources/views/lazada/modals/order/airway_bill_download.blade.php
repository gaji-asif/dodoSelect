...
<script>
    ...
    $("#btn_bulk_download_awb").on("click", function() {
        ...
        lazada_airway_bill_timer = setInterval(function() {
            checkIfPdfCanBeGenerated();
            getDowbloadableAirwayBillPdfs();
            $("#refresh_lazada_airway_bills_list_btn").removeClass("hide");
        }, 30000);
    });
    ...
    $('.__btnCloseModalOrderAirwayBill').on('click', function() {
        ...
        clearInterval(lazada_airway_bill_timer);
    });
    ...
    const generateInBulkPdf = () => {
        ...
        $.ajax({
            url: '{{ route("lazada.order.generate_airway_bill_in_bulk") }}',
            type: "POST",
            data: {
                'json_data': json_data,
            ...
            if(response.success) {
                ...
                setTimeout(function(){
                    ...
                    lazadaAirwayBillPdfIsGeneratingWithProgressBarHtml(percentage);
                    reloadOrderStatusList();
                }, 2000);
            } else {
    ...
    const getDowbloadableAirwayBillPdfs = () => {
        $.ajax({
            url: '{{ route("lazada.order.get_downloadable_airway_bill") }}',
            type: "GET",
            data: {
                '_token': $('meta[name=csrf-token]').attr('content')
            },
                ...
                if (typeof(response.data) !== "undefined") {
                    if (response.data.length > 0) {
                        $.each(response.data, function(index, pdf_info) {
                            let url = "{{ route('lazada.order.download_airway_bill', ':token') }}";
                            ...
                            html += '<a href="'+url+'" class="btn-action--green lazada_pdf_download_btn" data-token="'+pdf_info.token+'" target="_blank">Download Pdf</a>';
                            html += '</div>';
                            html += '</div>';
    ...
    $(document).on("click", ".lazada_pdf_download_btn", function(el) {
        let token = $(this).data("token");
        $("#downloadable_pdf_row__"+token).remove();
        $("#downloadable_pdf_row_missing_orders__"+token).remove();
        var el_len = $('.lazada_pdf_download_btn').length;
        if (el_len === 0) {
    ...
    const deleteShopeeAirwayBillPdf = (token) => {
        ...
        $.ajax({
            url: '{{ route("lazada.order.delete_airway_bill") }}',
            type: "POST",
            data: {
                "token": token,
                '_token': $('meta[name=csrf-token]').attr('content')
    ...
    const deleteAllShopeeAirwayBillPdfs = () => {
        $.ajax({
            url: '{{ route("lazada.order.delete_all_airway_bill") }}',
            type: "POST",
            data: {
                '_token': $('meta[name=csrf-token]').attr('content')
            }
    ...
    const checkIfPdfCanBeGenerated = () => {
        $.ajax({
            url: '{{ route("lazada.order.can_generate_airway_bill") }}',
            type: "POST",
            data: {
                '_token': $('meta[name=csrf-token]').attr('content')
            }
            ...
            if(response.success){
                if (typeof(response.data.can_generate) !== "undefined") {
                    if (response.data.can_generate) {
                        ...
                        $("#generate_download_pdf_for_bulk_orders_btn").prop("disabled", true);
                        if (typeof(response.data.percentage) !== "undefined") {
                            lazadaAirwayBillPdfIsGeneratingWithProgressBarHtml(response.data.percentage);
                        } else {
                            lazadaAirwayBillPdfIsGeneratingWithSpinnerHtml();
                        }
                    }
                }
            }
    ...
    const lazadaAirwayBillPdfIsGeneratingWithProgressBarHtml = (percentage=0) => {
        let html = '<progress id="file" value="'+percentage+'" max="100" style="height:20px;"> '+percentage+'% </progress>';
        html += '<button id="refresh_lazada_airway_bills_list_btn" class="btn-action--green ml-2 hide"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-repeat" viewBox="0 0 16 16">';
    ...
    const lazadaAirwayBillPdfIsGeneratingWithSpinnerHtml = () => {
        var img = '{{ asset("img/spinner.svg") }}';
        $("#download_pdf_for_bulk_orders_message").html("<img class='' style='height:70px;' src='"+img+"' alt=''>");
        $("#generate_download_pdf_for_bulk_orders_btn").prop("disabled", true);
    }

    $(document).on("click", "#refresh_lazada_airway_bills_list_btn", function() {
        checkIfPdfCanBeGenerated();
        getDowbloadableAirwayBillPdfs();
    });
</script>
